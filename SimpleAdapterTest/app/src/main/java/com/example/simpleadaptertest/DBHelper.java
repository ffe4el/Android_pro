...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String tableSQL = "create table tb_data (" +
                ...
                "content)";
        db.execSQL(tableSQL);
        db.execSQL("insert into tb_data(title, content) values ('어벤져스 엔드게임', '아이 엠 아이언맨')");
        db.execSQL("insert into tb_data(title, content) values ('범죄도시', '진실의 방으로')");
        db.execSQL("insert into tb_data(title, content) values ('스타워즈5', '암 유어 빠다')");
        db.execSQL("insert into tb_data(title, content) values ('아저씨', '이거 방탄유리야, 멍멍아')");
